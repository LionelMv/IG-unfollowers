...
class CheckFollowersView(APIView):
    def post(self, request):
        ...
        if not followers_data or not following_data:
            return Response({'error': 'Both files are required.'},
        ...
        try:
            ...
            followers = [item['value'] for entry in followers_json
                         for item in entry['string_list_data']]
            following = [item['value']
                         for entry in following_json['relationships_following']
                         for item in entry['string_list_data']]
            non_followers = [user for user in following
